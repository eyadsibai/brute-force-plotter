...
class TestManyCategories:
    ...
    @pytest.mark.edge_case
    def test_ignores_column_with_many_categories(
        self, sample_data_many_categories, temp_dir
    ):
...
class TestAllMissingValues:
    ...
    def test_plot_column_with_all_missing_values(self, temp_dir):
        ...
        try:
            ...
            pytest.fail(
                f"Plotting column with all missing values should not crash: {e}"
            )
...
class TestVariousDataTypeCombinations:
    ...
    @pytest.mark.edge_case
    def test_plot_only_numeric_columns(
        ...
        assert os.path.exists(
            os.path.join(interactions_dir, "col1-col2-scatter-plot.png")
        )
    ...
    def test_plot_only_categorical_columns(self, sample_categorical_data, temp_dir):
        ...
        interactions_dir = os.path.join(temp_dir, "2d_interactions")
        assert os.path.exists(
            os.path.join(interactions_dir, "category1-category2-bar-plot.png")
        )
    ...
    def test_plot_with_unicode_column_names(self, temp_dir):
        ...
        try:
            ...
            pytest.fail(
                f"Plotting with Unicode column names should not crash with non-font errors: {e}"
            )

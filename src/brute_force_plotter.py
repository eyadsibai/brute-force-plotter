...
def scatter_plot_with_hue(data, col1, col2, hue_col, file_name=None):
    ...
    if len(clean_data) == 0:
        logger.warning(f"No valid data points for scatter plot: {col1}, {col2}, {hue_col}")
        plt.text(0.5, 0.5, "No valid data points", ha='center', va='center', transform=plt.gca().transAxes)
        plt.title(f"{col1} vs {col2} by {hue_col}")
    else:
        sns.scatterplot(x=col1, y=col2, hue=hue_col, data=clean_data, palette="deep", s=100, alpha=0.7)
        plt.title(f"{col1} vs {col2} by {hue_col}")
...
def contour_plot(data, col1, col2, col3, file_name=None):
    ...
    if len(clean_data) < 4:
        logger.warning(f"Not enough data points for contour plot: {col1}, {col2}, {col3}")
        return
    ...
    try:
        ...
        # Create contour plot
        contour = ax.contourf(xi_grid, yi_grid, zi, levels=15, cmap='viridis', alpha=0.7)
        ax.contour(xi_grid, yi_grid, zi, levels=15, colors='black', alpha=0.3, linewidths=0.5)
        
        # Add scatter points
        ax.scatter(x, y, c=z, cmap='viridis', s=20, edgecolors='black', linewidth=0.5, alpha=0.8)
...
def multi_level_heatmap(data, col1, col2, col3, file_name=None):
    ...
    if len(unique_vals) > 10:
        logger.warning(f"Too many categories in {col3} ({len(unique_vals)}), limiting to first 10")
    ...
    for idx, val in enumerate(unique_vals):
        ...
        if ct.size > 0:
            ...
            ax.text(0.5, 0.5, f"No data for {col3}={val}", 
...
def grouped_bar_violin_plot(data, num_col, cat_col1, cat_col2, file_name=None):
    ...
    if len(unique_cat2) > 10:
        logger.warning(f"Too many categories in {cat_col2} ({len(unique_cat2)}), limiting to first 10")
    ...
    # Grouped violin plot
    ax2 = axes[1]
    sns.violinplot(x=cat_col1, y=num_col, hue=cat_col2, data=data, ax=ax2, split=False, inner='quartile')
